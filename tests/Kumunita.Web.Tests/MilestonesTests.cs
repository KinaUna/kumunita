...
public class MilestonesTests
{
    ...
    {
        foreach (string id in new[] { "M0", "M1", "M2", "M3", "GP", "ML", "ML-UI", "LS", "SP", "TZ", "DF", "TR", "RC", "GU", "GA", "RE", "TG", "PG", "UG", "M4", "EV-CAL" })
        {
    ...
    [Fact]
    public void EV_DWM_Is_The_Single_InProgress_Milestone()
    {
        var next = Milestones.All.Where(m => m.Status == Milestones.StatusNext).ToList();
        Assert.Single(next);
        Assert.Equal("EV-DWM", next[0].Id);
    }
