...
| `SampleData__Enabled` | **demo instances only (ADR 0055 / 0056)** — set `true` only for a **deployed demo site** (`Production` + a fresh DB), so first boot seeds the mock neighborhood. Omit on a real neighborhood (the seeder is then unreachable by construction). In the deploy posture the seed admin keeps its `SeedAdmin__` setup-token lane, the other demo accounts get random high-entropy passwords, and a single credentials summary is emailed to the seed admin's address — no weak credential is stored on the public instance. See OPS *Configuration reference* + README §Running. |
