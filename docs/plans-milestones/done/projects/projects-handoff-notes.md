...
## U12 — pinned seam tests (16 Core + 8 Web)

**Two new files, zero production-code changes, no gate (U13's):** (a) `tests/Kumunita.Core.Tests/ProjectServiceTests.cs` — the **16 Core pins** (design §2.7, verbatim names) over the frozen `IProjectService` against a live Postgres via the Core `PostgresFixture` (fresh-DB-per-test `BootStoreAsync` + `Plant`/`Services` helpers, the `EventServiceTests` shape): F1 `F1_TodoVisibleToAudienceMember` / `F1_TodoHiddenFromNonMember`, F2 `F2_TodoPlacedOnMultipleBoards_AppearsOnEach`, F3 `F3_BoardGate_TodoOnBoard_BoardDenies_HidesCard` / `F3_BoardGate_TodoOnBoard_BoardAllows_ShowsCard`, F4 `F4_MoveToStatusLane_SetsTodoStatus_Audited` (asserts the `todo.move_to_lane` `AccessAudit` row), F5 `F5_MoveToNullStatusLane_TodoStatusUnchanged`, F6 `F6_LaneAtMaxRefusesMoveIn` / `F6_LaneBelowMaxAllowsMoveIn`, F7 `F7_AssigneeHasStanding_UpdatesTodo`, F8 `F8_NonStandaloneActorRefused`, F9 `F9_SubtaskIsFullTodo_OwnStatusAssignee` / `F9_DeleteCascadesToSubtree` / `F9_ReparentToDescendant_Refused`, F10 `F10_CopyToBoard_Duplicates_OriginalUntouched` / `F10_MoveToBoard_RelocatesPlacement`. (b) `tests/Kumunita.Web.Tests/ProjectsControllerTests.cs` — the **8 Web pins** (design §2.8, verbatim names) over `ProjectsController` (NSubstitute seam, claim-based `KumunitaPrincipal`): `Todos_List_AudienceFiltered`, `Todo_Detail_SubtasksRendered`, `Board_Detail_LanesAndCards`, `Board_MoveLeftRight_TriggerStatusUpdate`, `Board_LaneAtMax_RefusesMoveIn`, `Board_CopyToBoard_Duplicates`, `Board_MoveToBoard_Relocates`, `Todo_AssignToUser_StandingGranted`. **Counts (no regressions, zero pre-existing tests changed):** **Web 381 + 8 = 389/389 pass**; **Core 720 + 16 = 736/736 pass** (Testcontainers Postgres). **`dotnet build Kumunita.slnx -c Debug` is green** (0 errors, 0 warnings). **Deviations (recorded per the unit-series drift rule):** (a) **`## U12 — Drift pause`** — the §2.7/§2.8 pin lists name **no** pinned drift-guard test, so per the escape-hatch rule **no guard test was written** (none invented to satisfy the "16+8+guard" reading); the formal §2.10 pinned-name drift guard is **U13's gate**, and AllKeys/registry parity is already pinned by the existing `KnownTranslationKeys_ParityTests` + `KwLRegistryConsistencyTests` (both still green here). Also noted: design §2.6's "18-key floor" is **stale** vs U11's 65 live `projects.*` keys (superset, already registered — not re-registered this unit). (b) **two Web detail tests boot a real Postgres store** (`Todo_Detail_SubtasksRendered`, `Board_Detail_LanesAndCards`) via the Web.Tests `PostgresFixture` (`BuildRealStoreAsync`, the `TagControllerTests` precedent) because `ProjectsController.TodoDetail`/`BoardDetail` run a Marten 9 async LINQ `Query<BoardItemPlacement>()…ToListAsync()` that casts to the internal `MartenLinqQueryable` and **cannot be NSubstituted**; the other 6 Web tests are pure NSubstitute.

## U13 — close (2026-09-23)

M5 flipped to **Done**, M6 to **In progress** — one change across `Milestones.cs` (M5 `StatusNext` → `StatusDone`, M6 `StatusPlanned` → `StatusNext`), `README.md` (Roadmap M5 "Done" / M6 "In progress" + the two body "next is M5" references), and `MilestonesTests.cs` (M5 joins the done set; `M5_Is_The_Single_InProgress_Milestone` → `M6_Is_The_Single_InProgress_Milestone`); `ARCHITECTURE.md` §3 tree (`Projects/` "not yet created" → "M5 ✓ (ADR 0067)") + the two §3 prose references + the §5 block (the `Project`/`ProjectTask`/`ProjectMember` sketch → the shipped `TodoItem` / `KanbanBoard` / `KanbanLane` / `BoardItemPlacement` field sets + the `ProjectService` surface). **Gate green (§2.9):** `dotnet build Kumunita.slnx -c Debug` green; Web **389/389** pass (closed loop + handoff — the 8 `ProjectsControllerTests` pins + `MilestonesTests`); Core **736/736** pass (part-vs-whole — the 16 `ProjectServiceTests` pins + the full M4 `EventServiceTests` + the existing post/announcement/page pin suites, nothing regressed). **ADR 0067: Accepted.** Lane closed — U00–U13 + this register + the rolling notes moved `in-progress/projects/` → `done/projects/`.
