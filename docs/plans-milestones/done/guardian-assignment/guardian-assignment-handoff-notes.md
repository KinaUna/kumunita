...
- **Docs flips (5 files):** `docs/ARCHITECTURE.md` `Identity/` line gains `FindSubjectByEmailAsync` (GA ✓, ADR 0038, G-A·2 no-leak shape) + `UserInfo/` line gains the `GuardianItem` record + `MembershipEditorModel.GuardianItems` field (GA ✓, ADR 0038, G-A·3 identical-in-kind pin); `docs/design/guardian-assignment-design.md` ends with `## GA — Closed (recorded) (2026-09-17)` (the 8-test gate all PASS, the 3 drift pauses carried from U06 named, the cross-lane red surfaced, the ADR 0038 §E non-decisions each named, the "M4/M5/M6 untouched" line); `README.md` §Roadmap gains the GA line (after GU, before M4, **Done**); `Milestones.cs` gains the GA line (`StatusDone`, after GU, before M4); `MilestonesTests.cs` "order" assertion gains `"GA"` (after `"GU"`, before `"RE"`) + the "Shipped_Milestones_Are_Marked_Done" list gains `"GA"`; the "single-in-progress" assertion is **unchanged** (M4 is still the single `StatusNext`).
- **Gate record (from the design doc § GA — Closed (recorded)):** 3 Core (U03) + 5 Web (U06) = **8/8 PASS** (closed loop + handoff + part-vs-whole all hold).
- **Drift pauses (carried from U06, named not silently resolved):** (i) `name="Email"` vs. `asp-for="Email"`; (ii) audit `ActorId` = assigned guardian (not assigning); (iii) Web.Tests now carries its own `PostgresFixture`.
- **Cross-lane red (SURFACED to user as a decision, NOT in GA deliverables, RESOLVED by user):** `GuardianViewModelsTests.MembershipEditorModel_Is_Exact_Four_Field_Projection` (GU lane, pre-existing) was red because U04 added `GuardianItems` (5th field). The user approved the fix (option 1): the GU projection test was renamed to `MembershipEditorModel_Is_Exact_Five_Field_Projection` and the expected field set updated to `{ "ChildId", "CommunityIds", "GroupIds", "GuardianItems", "PendingInvitations" }` (alphabetical). The GU lane's shape (the 4 original fields, in order) is preserved — the test now correctly reflects the 5-field model GA introduced. Build green; full `Kumunita.Web.Tests` suite 200/200 PASS (the 1-failure cross-lane red is cleared).
- **File moves (9):** `plan-guardian-assignment.md` + `guardian-assignment-u01-plan.md` … `guardian-assignment-u07-plan.md` (7 unit plans) + `guardian-assignment-handoff-notes.md` → `docs/plans-milestones/done/guardian-assignment/`; `in-progress/` confirmed empty afterward.
- **No build** (docs + moves only). **No commit / no stage** (per the user's instruction; the GU projection-test red is left as a decision for the user).

## Summary

GA is closed. The lane shipped one `IIdentityService` ADD (`FindSubjectByEmailAsync`), one `GuardianController` action (`Assign`), one view model + one record + one record field (`AssignGuardianForm` / `GuardianItem` / `MembershipEditorModel.GuardianItems`), two `Detail.cshtml` appends (the "other guardians" list + the assign form) + 8 localization keys, ADR 0038, and the roadmap trio flip. The 8-test acceptance gate (3 Core + 5 Web) is all **PASS**.

| Unit | Goal (one-liner) | Tests (pass/red) | Deviations |
|------|------------------|------------------|------------|
| U01 | Design doc + ADR 0038 (the pinned contract) | — (no code) | none |
| U02 | `IIdentityService.FindSubjectByEmailAsync` + impl | — (no test) | none |
| U03 | 3 Core seam tests | 3 / 3 PASS | none |
| U04 | `AssignGuardianForm` + `GuardianItem` + `MembershipEditorModel.GuardianItems` | — (no test) | left `CS7036` for U05 (pinned-contract exactness, not a default value) |
| U05 | `GuardianController.Assign` + `ActiveGuardiansAsync` + `Detail`'s `GuardianItems` arg | — (no test) | none |
| U06 | `Detail.cshtml` two appends + l10n keys + 5 Web tests | 5 / 5 PASS | (i) `name=` vs. `asp-for=`; (ii) audit `ActorId` = assigned; (iii) Web.Tests carries its own `PostgresFixture` |
| U07 | ARCH flip + `## GA — Closed (recorded)` + roadmap trio + move to `done/` | 200/200 Web PASS (after cross-lane GU test fix) | cross-lane GU projection-test red resolved (GU test updated to expect 5 fields, renamed to `…_Five_Field_Projection`) |

**ADR 0038 §E non-decisions (each named, carried forward):** no remove path; no acceptance/consent step; no bulk assign; no self-assignment (refused, not a lane); no second audit verb; no email notification to the assigned guardian. Each re-litigates as an ADR 0038 amendment, not a toggle.
