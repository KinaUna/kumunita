...
- **Unit register line:** `docs/plans-milestones/done/plan-m3b-moderation.md` → U5 (line 208).
...
- **Exit:** `run_build` green on `Kumunita.Core` (verified); handoff note appended to `docs/plans-milestones/done/m3b-handoff-notes.md` as `## U5 — Assign / unlock / resolve + Via=Report read branch` **before** any follow-up action.
...
- `D:\repos\Kumunita\src\Kumunita.Core\Authorization\AuthorizationService.cs`
  ...
  modify it).
- `D:\repos\Kumunita\docs\plans-milestones\done\m3b-handoff-notes.md`
...
- **The "same `IDocumentSession` transaction" pin for the
  ...
  (`docs/plans-milestones/done/m3b-handoff-notes.md`, `## U5`).
...
4. **Append the handoff note** to
   `docs/plans-milestones/done/m3b-handoff-notes.md` — the
