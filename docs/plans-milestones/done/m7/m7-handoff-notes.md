...
## U05 — Close (README / Milestones.cs / MilestonesTests / ARCHITECTURE.md + this Summary)

- **Milestone flipped:** `Milestones.cs` — `M7` `StatusNext` → `StatusDone`,
  `M8` `StatusPlanned` → `StatusNext` (no other entry touched). README § Status
  paragraph — "**next is M7** — pagination & filtering" → "**M7 is done** —
  pagination & filtering (… ADR 0090); **next is M8** — search", then M9–M13.
  README § Roadmap — the **M7** row → **Done (ADR 0090)**, the **M8** row →
  **In progress.** (M9–M13 unchanged.)
- **MilestonesTests updated (the id flip, in three places):** the test pins M7
  *by id* — (1) `Shipped_Milestones_Are_Marked_Done` now includes `M7` in the
  done list; (2) the single-in-progress test renamed
  `M7_Is_The_Single_InProgress_Milestone_And_M8_Through_M13_Are_Planned` →
  `M8_Is_The_Single_InProgress_Milestone_And_M9_Through_M13_Are_Planned`, with
  the expected in-progress id `M7` → `M8` and the planned list `M8–M13` →
  `M9–M13`. (This is the "one-line test edit, in scope" the plan anticipated;
  it touches the id pin + the method name, not any order/count assertion.)
- **ARCHITECTURE.md note (the plan's exact text):** added the M7 (ADR 0090)
  shared-pagination-idiom paragraph to § 2 "Solution layout" (the "shape of
  the code" section — `PagedViewModel` + `_Pager` partial + the `HasMore`
  signal; a new paged surface adds the D1/D3 seam signal + drops in the D5
  `_Pager`, never re-inventing a pager).
- **Validation:** `dotnet build Kumunita.slnx -c Debug` green; `MilestonesTests`
  green (the reliable path:
  `dotnet exec tests\Kumunita.Web.Tests\bin\Debug\net10.0\Kumunita.Web.Tests.dll
  -class Kumunita.Web.Tests.MilestonesTests`). README + `Milestones.cs` agree
  (M7 **Done** in both, M8 **Next**/In progress in both). No code beyond the
  `Milestones.cs` flip + the `MilestonesTests` id flip.
- **Known pre-existing issue (NOT fixed here, out of U05 scope):** the 6
  `ProjectsControllerTests` NREs flagged from U03 (`SeedGoalPickerAsync` /
  `SeedBlockerPickerAsync` — the tests stub only `GetProjectAsync`, so U01's
  new page-record seams `ListGoalsAsync` / `ListProjectsAsync` /
  `ListBoardsAsync` return a null page record under NSubstitute → `.Items`
  NREs). They do not block `MilestonesTests`; fixing them (stub the page
  records) is the follow-on for the next agent.

## Summary

M7 (pagination & filtering) is **closed**. The one canonical paging contract
(ADR 0090) is the locked reference: every paged Core seam reports `HasMore`,
`FeedResult.Total` is the candidate-set count (never a viewer-facing total),
and the Web renders it through the single `PagedViewModel` + `_Pager` partial.

| Unit | Goal | Tests (pass) | Drift |
|---|---|---|---|
| U00 | Design doc + ADR 0090 (the sign-off gate) | — (no code, no build) | none (5 refinements locked in the design doc's drift log §10) |
| U01 | `HasMore` on every paged seam + `Total` correction + 3 D6 seams + 12 seam tests | 12/12 (`M7PaginationSeamTests`) | 1 — the `out bool` pin is CS1988-illegal on `async`; the page-record return (`EventPage`/`TodoPage`/`BoardPage`/`GoalPage`/`ProjectPage`) is the only legal vehicle (design log §U01); the U01 §7.2a call-site compile pass |
| U02 | `PagedViewModel` + `_Pager` partial + the two `kw-l` keys | 4/4 (`PagedViewModelTests`) + 7/7 parity | none |
| U03 | Wire the pager into the 7 existing paged routes (8 instances) + D2 view-text | 3/3 (`M7PagerWiringTests`) | 1 — the `PostService` is `sealed` (NSubstitute can't proxy it) → the two `PostsFeed_*` tests are data-shape pins over a `FeedResult` mirroring the controller's exact wiring; the `GroupsDetailViewModel` shape-pin gained `PagerPosts`/`PagerEvents` |
| U04 | Wire the 2 newly-paged routes (3 sections) + D7 filter-reset | 3/3 (`M7NewlyPagedTests`) + 49/49 + 8/8 no-regression | 1 — both controllers fall back to the unmodified non-paged seam when the page record's `Items` is null (the test-double escape; the paged seam remains the app's read) |

**M8 handoff:** M7 is closed; the paging contract is **ADR 0090**; the filter
set is frozen (D9) — **M8 search is the next filter surface (text-based)**,
and it reuses the `HasMore` signal + the `_Pager` partial (no new pager
idiom). A new paged surface (or a text-search feed) adds the `HasMore` signal
to its seam (the D1/D3 shape) and drops in `_Pager` (the D5 shape); it does
not re-invent a pager. The 6 pre-existing `ProjectsControllerTests` NREs
(U01's page-record seams under NSubstitute) remain open — the next agent
should stub those seams' page records.
