...
## Lane closed — U08 (RE→future-lane handoff artifact)

**Date:** 2026-09-15

**The RE lane is closed.** ADR 0031 is **Accepted** (U01 drafted it, U08
accepted it — the separation held). The **`RE` named lane** is landed in
`Milestones.cs` + the README **Roadmap** + `MilestonesTests.cs` (a **named**
lane — the `GP`/`RC` precedent — **not** a renumber; M4/M5/M6 stay
Events / Projects / Portability; M4 remains the single `status-next`).
The **`rc.editor.*`** `<kw-l>` keys are registered in
`KnownTranslationKeys` (the 11-key canonical set: `bold`, `italic`, `code`,
`h1`, `h2`, `h3`, `list`, `olist`, `link`, `image`, `preview` — **no
`.quote`**, per U1's drift pause: there is no blockquote button because the
frozen `MarkdownRenderer` has no blockquote branch). The ADR index
(`docs/adr/README.md`) gains the **0031** row. `dotnet build` + `dotnet
exec Kumunita.Web.Tests.dll` are green (recorded under U08's verify).

### U08's close-time doc correction (delegated by U7)

U7's handoff named one **doc digit typo** as an "Action for U08 (close)":
`ApplyToggle_Bold_WrapsSelection_AndPreservesCaret` was pinned to selection
`[7,12]`, but the frozen `rich-editor.ts` `applyToggle` (and the RE2 FACES'
"selection preserved" intent) produce **`[8,13]`** — `world` in
`"hello **world**"` at `[8,13]`. U7 verified this by running the module in
node. U08 corrected `[7,12]` → `[8,13]` in **both** records —
`docs/design/rich-editor-design.md` §Pinned seam tests #6 and
`plan-rich-editor.md` §Pinned seam tests #6 — a one-line doc fix, **no code
change**. The `RichEditorTests` assertion (which U7 wrote against the
module-faithful `[8,13]`) is now the source of truth and the records agree
with it.

### The gated image lanes — the first RC follow-ups (the register §When a
future lane wants more)

The **text toolbar + live preview are fully on at every surface** (10 of the
11 button-label keys, no image button) — the bodies already render through the
one `MarkdownRenderer`, so the authoring UX is complete everywhere. The **only
gated** control is the **Image button**, **OFF** on four surface-groups because
RC's image lane is incomplete there — each a **distinct RC seam RE·3 does not
invent**. This is the primary RE→future-lane handoff: a future lane that builds
the missing RC seam on a surface un-gates that surface's image button as a
**one-attribute removal of `data-rich-editor-no-image`** (plus adding
`data-image-target` on the textarea and, where RC's write lane was the gap,
populating the `ImageIds` field) — **no RE code**.

| Surface (view) | Missing RC seam (the gate reason) | RC seam id |
|---|---|---|
| **Post Edit** — `Views/Posts/Edit.cshtml` | `UpdatePostAsync` does **not** populate `post.ImageIds` (the write lane); serve branch exists (post owner) | RC drift pause (c) |
| **Group Edit** — `Views/Groups/Edit.cshtml` | `UpdateGroupPostAsync` does **not** populate `post.ImageIds` (the write lane); serve branch exists (post owner) | RC drift pause (c) analog |
| **Announcement New + Edit** — `Views/Announcement/{New,Edit}.cshtml` | `ContentImageController.Serve` has **no announcement branch** (inert-404); the write lane (`Announcement.ImageIds`) is wired | RC U03 drift pause |
| **Reply New/Edit** — `Views/{Posts/Detail,Groups/PostDetail,Announcement/Detail}.cshtml` (all `name="body"` reply textareas) | **both** seams missing: `CreateReplyAsync`/`UpdateReplyAsync` do **not** populate `PostReply.ImageIds` (drift pause (a)) **and** `ContentImageController.Serve` has **no reply branch** (inert-404) | RC drift pause (a) + RC U03 |

Each gate's **exact seam + view path + the `data-rich-editor-no-image`
attribute** are the handoff — a future RC lane names them and the un-gate is
one attribute (and the write-lane population where that was the gap). RE ships
a working Image button **only** where RC can actually serve the image (Post
New, Group New, the static-page editor); it does **not** ship a button that
would 404.

### The named deferrals (the register §When a future lane wants more)

- **Undo/redo** — the browser's native `Ctrl+Z` on the textarea is the floor; a
  future lane names a custom history API.
- **WYSIWYG `contenteditable` / third-party editor (ProseMirror / Quill /
  Tiptap)** — **hard non-negotiable out** (D1, ADR 0031): it breaks `tsc`-only
  + the no-HTML-round-trip stance + RC's `ImageIds` source parse. Not a
  deferral; a separate-architecture decision.
- **Blockquote (`> `)** — **cut in U1's drift pause** (no button, no `rc.editor.quote`
  key). A future lane must extend **both** `MarkdownRenderer` **and** the
  client `renderPreview` in the same commit (RE·2 parity holds only if both
  move together), and add the `rc.editor.quote` key in the same commit.
- **Tables / footnotes** — RC's "intentionally out of scope" set; a lane adds
  them to `MarkdownRenderer` **and** the client preview **together**.
...
### RC debt not folded into RE

- **`rc.markdown_hint`** is **not** registered in `KnownTranslationKeys`.
  U0 (the kickoff) and U6 both flagged it as **RC's debt** (RC U05's hint
  deferral — the placeholder key was never named). U08 registered **only** the
  `rc.editor.*` set and **left `rc.markdown_hint` out** — it is RC's, not RE's.
  A future RC follow-up should register it (a one-key addition) if the hint
  paragraph is ever made a `<kw-l>` key.

### The frozen RC base this lane builds on (unchanged, R·1–R·7)

The one `MarkdownRenderer` (read path), `ContentImageIds` (the `ImageIds`
parse), the four `ImageIds` fields, the `/content-image` routes,
`IMediaStore`, the `.rc-body`/`.rc-image` CSS, and `insert-image.ts` are all
**unchanged** by RE. RE added **only** the client-side authoring surface:
`client/lib/rich-editor.ts`, the `.rc-editor-*` CSS block, the toolbar +
preview markup on the composer views, and the `rc.editor.*` keys (U08).
**No** `.csproj` / `package.json` / route / `AccessAction` / `IMediaStore`
change (RE·3). The lane regresses nothing RC shipped — the acceptance gate
(`RichEditorTests`, 11 tests) + the inherited RC `MarkdownRendererTests` (7) +
`ContentImageUploadTests` (4) + `ContentImageServingTests` (4) all pass
together under `dotnet exec`.
