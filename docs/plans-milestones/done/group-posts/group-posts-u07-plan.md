...
- `src/Kumunita.Web/Controllers/GroupsController.cs` — append the 4 group-post actions **under the existing `/groups/{id}` route** (mirroring `PostsController`'s pattern, not inventing a new verb-style):
  ...
  - `GET /groups/{id}/posts/{postId}` → `GetGroupPostAsync` → `View("PostDetail", GroupPostDetailViewModel)` (`Views/Groups/PostDetail.cshtml`; 404 on Deny — the fail-closed shape, Core maps a membership Deny to `Post = null`; the register's "Non-member: 404" pin)
