...
Then append a `## U2 — Design doc Part 2` handoff section to `docs/plans-milestones/done/m3b-handoff-notes.md` with the pinned signature count, seam-test count, and a one-liner telling U3 to touch `src/Kumunita.Core/Posts/Post.cs` first for the `PostStatus` enum + `Status` field.

## Key Files
- `docs/design/m3b-moderation.md` — append Part 2.
- `docs/plans-milestones/done/m3b-handoff-notes.md` — append the `## U2` section.
...
-  **Append `## U2 — Design doc Part 2` to `docs/plans-milestones/done/m3b-handoff-notes.md` (pinned signature count, seam-test count, the U3 pointer).**
