...
- **(c) `Report` registered, `Status` nullable, no index, no seed:** table-in-M3 / flow-in-M3b (design doc §2.2 + §2.6 flag). No write lane, no surface, no tests in M3 — U3's POCO is the *registration artifact only*; the M3b unit that owns the file/assign/unlock/resolve flow will add the `(PostId, Status)` business-key index in its own `M3bDocTypes` (or an extension to `M3DocTypes`, M3b's choice) and set the `Status` column.
