...
## Post-M3 — Composer audience picker → shared partial (2026-09-10)

- **What changed:** the U8 deviation (i) above — *"a future unit decides whether to extract the shared partial under all audience surfaces"* — is now closed, with the decision: **yes, extract**.
  - **New shared partials** (single rendering site for the "Who to grant to" checkbox UI + its sync script):
    - `Views/Shared/_GrantPickers.cshtml` (`@model GrantPickersModel`) — the two checkbox sections (users + groups) with select-all tri-states and "N of M selected" counters, the empty-pool notes, and the hidden `d-none` `textarea[name="{EditorName}.Grants"]` (with `data-editor-name`) that remains the **only form-bound grants field** (the M2/F13 e2e pin shape, now generalized from the `Visibility`/`ContactVisibility` editor names to any surface).
    - `Views/Shared/_GrantPickerScripts.cshtml` — the client-side sync IIFE (orphan-grant preservation, `(Kind,Id)` de-duping, box↔textarea two-way sync), moved **verbatim** out of `Profile/Edit.cshtml`'s `@section Scripts`.
    - `Models/GrantPickersModel.cs` — the partial's `@model` record (`EditorName` / `Editor` / `Users` / `Groups`), so the surface view models (`ProfileEditViewModel` vs `PostComposeViewModel`) never meet the partial's model-type check — which is precisely the Razor model-type mismatch U8 (i) flagged as the reason the composer inlined a copy instead of reusing M2's `@model ProfileEditViewModel` partial.
  - **Profile/Edit (unchanged behavior):** `_AudienceEditor.cshtml` now builds a `GrantPickersModel` (options still read from the `Visibility_*` / `ContactVisibility_*` `ViewData` keys set by `ProfileController`) and renders `<partial name="_GrantPickers">`; the inline checkbox markup + local `renderGrantCheckboxSection` live in the shared partial now; `Edit.cshtml`'s script section renders `<partial name="_GrantPickerScripts">`.
  - **Composer (the user-visible change):** `Posts/New.cshtml` renders the same picker (`EditorName = "Audience"`) instead of the hand-typed JSON textarea; `PostsController` (GET *and* POST `New()`, so both re-render paths show options) seeds `ViewData["Audience_Users"]` / `ViewData["Audience_Groups"]` with the **same M2 single-source rules** as `ProfileController.SeedGrantPickerOptionsAsync` (verified, unblocked, self-excluded users + all groups, label-ordered).
  - **Pins preserved:** `AudienceEditorModel` / `BuildAudience()` single-deserializer shape untouched (U8 (b) reuse discipline); no grant options ever as form-bound model properties (the `ViewData`-not-`ViewBag` convention — statically-typed `ViewDataDictionary` only); the e2e pins are unaffected (`textarea[name="Visibility.Grants"]` per `e2e-m2.spec.ts` still renders; the composer e2e asserts only `Title`/`Body`, never a grants field — no `aud-grants` selector exists anywhere).
  - **Verification:** full-solution build green (0 errors; one pre-existing CS8602 at `New.cshtml:144` — `Model.Audience.IsEmpty` against the nullable `Audience` property — surfaced as a warning and fixed with a null-guard that preserves the existing alert behavior); `Kumunita.Web.Tests` **60/60 passed** in-process (xunit.v3 runner; per `AGENTS.md`, VS Test Explorer discovery is broken on this machine). `Kumunita.Core.Tests` not run here: web-layer-only change (views + `PostsController`), no `Kumunita.Core` edit, and the suite requires Docker/Testcontainers.
  - **Note for M3b:** this was a UI-shape extraction, not a contract change — no seam, no document, no ADR touch; the M3b deferral list above is unaffected.
