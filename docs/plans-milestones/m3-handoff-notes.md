...
- **(d) Files touched + build + DI:** `src/Kumunita.Core/UserInfo/IUserInfoService.cs` (M3 ADD appended above the M2 additions block, doc-comment verbatim from design doc §2.1 — anchored to C-M3·2 + ADR 0006-E lane + strong-consistency (C4) + references `F9_CandidateFilterEmitsNoAuditRow`), `src/Kumunita.Core/UserInfo/UserInfoService.cs` (implementation, one new method in the read-paths section after `GetProfilesAsync`), `tests/Kumunita.Core.Tests/UserInfoServiceTests.cs` (one new `[Fact]` after `GetProfilesAsync_VerifiedOnly_Filters`). `run_build` green on both `Kumunita.Core` and `Kumunita.Core.Tests` (the two touched projects). DI: no registration change — `IUserInfoService → UserInfoService` already rides the transient registration in `src/Kumunita.Core/DependencyInjection.cs` (M2 U3 precedent); the new interface method does not alter the type identity, so the existing registration binds it.
