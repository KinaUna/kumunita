...
| **Per-unit** | `media-u#-plan.md` (under `in-progress/`) — the full self-contained unit (Understanding/Assumptions/Approach/Key files/Risks/Steps), one per U#. Authored alongside by the plan author so a fresh agent can run it standalone. | plan author (done) + unit (verifies) |
