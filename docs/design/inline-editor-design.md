...
| **IE2** | The resident clicks the toggle button (the far-right toolbar button, labeled `</>`) → the source `<textarea>` becomes visible (the class is removed) and the pane hides. The resident can now hand-type Markdown or use the toolbar's splice buttons on the visible source. The button's label changes to "Preview". | IE·1 |
...
| **Label swap mechanism** | The `<kw-l>` is **server-rendered** (LocalizeTagHelper). A client-side `key` attribute change does **not** live-update the label. The `key` swap is the pin (HTML-source correctness + a11y); the `lastChild.textContent` swap (the `<kw-l>` element's text) is the **load-bearing** live path. The two labels are hardcoded in the binder: `"</>"` (show source) and `"Preview"` (show preview). |
| **`renderPane` reuse** | The existing `textarea.addEventListener('input', renderPane)` wiring is **untouched**. The binder's `renderPane` function is **reused verbatim** (not re-wired, not re-created). The pane's visibility is controlled by the CSS class (`rc-editor-source-hidden` hides the textarea; the pane is a separate grid row that stays visible unless the `rc-editor-pane-active` class is added for per-state styling). |
