...
/// <summary>
/// A **user-added translation** of a <see cref="KanbanBoard"/> into a language
...
/// <para>
/// One row per (board, language) pair — the
/// <c>(BoardId, LanguageCode)</c> unique index
...
/// translation carries its own <see cref="Title"/> (optional — the base board's
/// <see cref="KanbanBoard.Title"/> is the fallback when absent) and
/// <see cref="Body"/> (optional — M5 boards are title-usable without a body,
...
/// board's <b>creator</b>, a <b>Translator</b> (ADR 0021), or a
...
/// board, a translation has <b>no own audience</b> — its visibility inherits
/// the board's single <c>Read</c> decision. A translation row that is not under
/// a board the viewer may read is simply unreachable (the Web reads it only
/// after <see cref="ProjectService.GetBoardAsync"/> returned the board).
/// <para>
...
/// </para>
/// </summary>
public sealed class BoardTranslation
{
    public string Id { get; set; } = string.Empty;

    /// <summary>The parent <see cref="KanbanBoard"/> this translation renders.</summary>
    ...
    /// — a supported, enabled language). Distinct from the board's authored-in
    /// <see cref="KanbanBoard.LanguageCode"/>: that is the language the board
    /// was written in (the base); this is the language the translation renders
    /// it into.
    /// </summary>
    public string LanguageCode { get; set; } = string.Empty;

    /// <summary>The translated title (optional — the base board's title is the
    /// fallback when this is absent).</summary>
    public string? Title { get; set; }

    /// <summary>The translated body (optional — M5 boards are title-usable
